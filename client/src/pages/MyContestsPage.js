...
import BottomNav from "../components/BottomNav"; // Assuming BottomNav is in this path
...
const MyContestsPage = () => {
    ...
    const fetchData = async () => {
      ...
      try {
        const [userResponse, contestsResponse] = await Promise.all([
          ...
          api.get("http://localhost:5000/api/contests/my-contests"),
        ]);
  ...
  // --- [START] UPDATED CONTEST FILTERING LOGIC ---
  const liveContests = [];
  ...
  myContests.forEach((contest) => {
    // Rule 1: If a leaderboard exists, the contest is ALWAYS considered "Finished".
    if (contest.leaderboard && contest.leaderboard.length > 0) {
      finishedContests.push(contest);
    }
    // Rule 2: If no leaderboard is present, use the time-based logic.
    else {
      const startTime = new Date(contest.startTime);
      const oneHourAgo = new Date(now.getTime() - 60 * 60 * 1000);

      if (startTime > now) {
        // If the start time is in the future, it's "Upcoming".
        upcomingContests.push(contest);
      } else if (startTime <= now && startTime > oneHourAgo) {
        // If it started within the last hour, it's "Live".
        liveContests.push(contest);
      } else {
        // If it's older than an hour (and has no leaderboard), it's "Finished".
        finishedContests.push(contest);
      }
    }
  });
  ...
  const contestsToDisplay =
    activeTab === "Live"
      ...
      : activeTab === "Upcoming"
        ? upcomingContests
        : finishedContests;
  // --- [END] UPDATED CONTEST FILTERING LOGIC ---
