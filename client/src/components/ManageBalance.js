                ...
                <div className="bg-gray-800/50 p-6 rounded-xl">
                    <div className="flex items-center gap-3 border-b border-gray-700 pb-4 mb-4">
                        ...
                        <div>
                            ...
                            <p className="text-gray-400 text-sm">Current Points: <span className="text-green-400 font-semibold">{foundUser.points}</span></p>
                        </div>
                    </div>
                    ...
                    <form onSubmit={handleUpdateBalance}>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <label className="block text-gray-300 text-sm font-bold mb-2">Action</label>
                                <div className="flex gap-4 p-2 bg-gray-900 rounded-lg">
                                    <button type="button" onClick={() => setAction('add')} className={`flex-1 py-2 rounded-md transition-colors ${action === 'add' ? 'bg-green-600 text-white' : 'bg-gray-700 text-gray-300'}`}>Add Points</button>
                                    <button type="button" onClick={() => setAction('subtract')} className={`flex-1 py-2 rounded-md transition-colors ${action === 'subtract' ? 'bg-red-600 text-white' : 'bg-gray-700 text-gray-300'}`}>Withdrawal</button>
                                </div>
                            </div>
                            <div>
                                <label className="block text-gray-300 text-sm font-bold mb-2" htmlFor="points">Points</label>
                                <input
                                    id="points"
                                    type="number"
                                    value={points}
                        ...
                        <button type="submit" disabled={loading} className="mt-6 w-full bg-gradient-to-r from-orange-500 to-purple-600 text-white font-bold py-3 px-4 rounded-lg transition-all duration-300 disabled:opacity-50">
